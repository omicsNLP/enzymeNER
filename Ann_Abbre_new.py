...
def search_ase(text, word):  
    ...
    if search_list:
        ...
        for item in search_list:
            ...
            if pstart >= 0 and item.split()[0].lower() in words: #text.lower().split(): # greedy 
                if len(item) > maxl:
                   maxl = len(item)
...
def abbre_enzyme_list(filename):
    abbre_path = 'Auto-CORPus/output-Microbiome/Abbreviations/' + filename.replace('bioc', 'abbreviations')
    ...
    for j in range(len(abbreviation['documents'][0]['passages'])):
        ...
        if matchword != '' and abbre_search_enzyme(item['text_long_1'], matchword):
            ...
            enzyme_long[item['text_short']]=item['text_long_1']
    return enzyme_short
...
def annotate_abbre(abbre, text, offset, index):
    ...
    annotation_dict['id'] = str(id_num)
    if enzyme_long[abbre].lower() in ec_number_list:
        annotation_dict['infons']['identifier'] = ec_number_list[enzyme_long[abbre].lower()]
    else:
...
if __name__ == "__main__":
    ...
    print_path = ("Auto-CORPus/printnew-Microbiome.txt")
    ...
    for filename in os.listdir('Auto-CORPus/output-Microbiome/Main_Text'):
        if re.match('(.*).json$', filename):
            file_path = 'Auto-CORPus/output-Microbiome/Main_Text/' + filename
                    ...
                    for word in text['text'].split():
                        if word.find(item) >= 0:
                            ...
                            if word[0] == '(' and word[-1] == ')':
                                word = word[1:-1]
                            if word == item:
                                annotation.append(annotate_abbre(word, text['text'], offset, index))
                            index += 1
        ...
        writein_path = "Auto-CORPus/Annotated_Corpus/Microbiome-Abbre/" + filename.rstrip('.json') + '_' + 'annotated.json'
