...
class TaboListem(object):
    ...
    def __init__(self):
        ...
        """
        # bert_pretrain_path = "W:/Imperial/Project 2/TrainingSet/BioBertLarge/biobert-base-cased-v1.2"#!! can't do lowercase?
        bert_pretrain_path='dmis-lab/biobert-base-cased-v1.2'#"allenai/scibert_scivocab_cased"#'dmis-lab/biobert-base-cased-v1.2'##'bert-base-cased'
    ...
    def _prepare_seqs(self, seqs, verbose=True,save_path='Seqs.npy'):
        ...
        for seq in seqs:
            ...
            seq["ni"] = np.array([ni_cls]+[self.fzr.num_feats_for_tok(i) for i in tok_rep]+[ni_sep])
        if verbose:
    ...
    def build_model(self):
        ...
        transformer_model = transformers.TFBertForTokenClassification.from_pretrained(
            self.bert_pretrain_path, from_pt=True) # bert-base-cased
    ...
    def train(self, textfile, annotfile, runname):
        ...
        jf = open("tabolistem_BioBERT_%s.json" % runname, "w", encoding="utf-8")
        json.dump(outjo, jf)
        jf.close()
        ...
        for epoch in range(10):
            ...
            model.save_weights("./BioBERTModels/epoch_%s_%s_weights" %
                                    (epoch, runname))
    ...
    def test(self, textfile, annotfile):
        cr = CorpusReader(textfile, annotfile)
        #seqs = cr.train + cr.test
        #seqs = cr.trainseqs + cr.testseqs
        ...
        for size in sizes:
            ...
            while batch_counter<len(train_l_d[size]):
                ...
                y_true.extend(ty[0].argmax(axis=1))
                y_test.extend(np.array(Y[0], dtype = int).argmax(axis=1))

        cm = mx.confusion_matrix(y_true, y_test)
        print(cm)
        f1_weighted = f1_score(y_true, y_test, average = 'weighted')
        f1_micro = f1_score(y_true, y_test, average = 'micro')

        print(classification_report(y_true, y_test))
        print('macro-F1-score={}'.format(f1_weighted))
        print('micro-F1-score={}'.format(f1_micro))
